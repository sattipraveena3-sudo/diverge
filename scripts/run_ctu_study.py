...
def evaluate(
    table: pd.DataFrame, columns: list[str], seed: int = 42
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run leakage-safe record-level CTU-CHB DiVerge study"
    )
    ...
    report: dict[str, dict] = {
        ...
        "outcome_prevalence": float(table.outcome.mean()),
    }
    ...
    report["paired_tests"] = {
        "full_vs_raw_auprc_p": paired_wilcoxon(
            fold_scores["raw_only"]["auprc"], fold_scores["full"]["auprc"]
        ),
        "full_vs_divergence_auprc_p": paired_wilcoxon(
            fold_scores["divergence_only"]["auprc"], fold_scores["full"]["auprc"]
        ),
    }
