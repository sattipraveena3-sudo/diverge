...
from .divergence import rolling_cross_correlation_divergence, rolling_residual_divergence, rolling_dtw_divergence
...
def build_features(frame: pd.DataFrame, window: int = 60) -> pd.DataFrame:
    ...
    work["dtw_div"] = rolling_dtw_divergence(work["hr_z"], work["spo2_z"], max(12, window // 2), stride=5)
